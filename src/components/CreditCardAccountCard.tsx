...
export const CreditCardAccountCard: React.FC<CreditCardAccountCardProps> = ({
  ...
  return (
    ...
    >
      <div className="p-3.5 sm:p-4">
        ...
        <div className="flex items-center justify-between gap-2 mb-2">
          <div className="flex items-center gap-1.5">
            <div className="w-6 h-6 rounded-md bg-[#9E432A]/10 text-[#9E432A] flex items-center justify-center shrink-0">
              <CardIcon className="w-3.5 h-3.5" />
            </div>
            <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#9E432A] bg-[#9E432A]/10 px-1.5 py-0.5 rounded-md border border-[#9E432A]/20">
              Fatura de Cartão
            </span>
          </div>
        ...
        <div className="mt-1.5 space-y-1.5">
          {hasPreviousPending ? (
            <div className="rounded-xl bg-[#FDFCF9] border border-amber-200/80 p-2.5 space-y-1.5">
        ...
        <button
          ...
          className="mt-2 w-full flex items-center justify-between px-2.5 py-1.5 bg-[#FAF8F5] hover:bg-[#F2EEE7] border border-[#E2DDD3] rounded-xl text-xs font-medium text-stone-700 transition-colors cursor-pointer"
        >
          <span>
            {isExpanded
      ...
      {isExpanded && (
        <div className="border-t border-[#EAE5DC] bg-[#FAF8F5]/50 p-3 space-y-3 animate-in slide-in-from-top-1 duration-150">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 text-xs font-semibold text-[#9E432A] hover:text-[#88361F] transition-colors cursor-pointer"
              >
            ...
            {items.length === 0 ? (
              <div className="py-2.5 text-center text-xs text-stone-400 bg-white rounded-lg border border-[#E2DDD3]">
              ...
              <div className="space-y-1.5 max-h-56 overflow-y-auto pr-0.5">
                {items.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center justify-between p-2 rounded-lg bg-white border border-[#E8E3DA] text-xs gap-2 group/item hover:border-[#CBC4B5] transition-colors"
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        {item.isInstallment && item.installmentInfo && (
                          <span className="text-[10px] font-semibold text-[#9E432A] bg-[#9E432A]/10 px-1.5 py-0.5 rounded border border-[#9E432A]/20 inline-flex items-center gap-1">
              ...
              <div className="space-y-1.5">
                {previousPendingInvoices.map((inv) => (
                  <div
                    ...
                    className="flex items-center justify-between p-2 rounded-lg bg-[#FDFCF9] border border-amber-200 text-xs"
                  >
                    <div>
