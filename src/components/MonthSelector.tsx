...
export const MonthSelector: React.FC<MonthSelectorProps> = ({
  ...
  return (
    <div ref={containerRef} className={`relative flex items-center gap-2 ${className}`}>
      <div className="flex items-center bg-white/95 border border-[#E2DDD3] rounded-xl shadow-2xs px-1 py-1 hover:border-[#D5CFBF] transition-colors">
        <button
          id="btn-prev-month"
          type="button"
          onClick={handlePrev}
          title="Mês anterior"
          className="p-1.5 text-stone-400 hover:text-stone-700 hover:bg-[#F4F1EA] rounded-lg transition-colors cursor-pointer"
        >
        ...
        <button
          ...
          className="px-2.5 py-1 text-center min-w-[145px] sm:min-w-[160px] flex items-center justify-center gap-1.5 hover:bg-[#FAF8F5] rounded-lg transition-colors cursor-pointer group"
        >
      ...
      {!isCurrentSystemMonth && (
        <button
          ...
          className="text-[11px] font-semibold text-stone-600 hover:text-stone-900 bg-white/90 hover:bg-[#FAF8F5] border border-[#E2DDD3] px-2.5 py-1.5 rounded-xl transition-all cursor-pointer inline-flex items-center gap-1 shadow-2xs"
        >
          <Calendar className="w-3 h-3 text-[#9E432A]" />
          <span>Hoje</span>
        </button>
      )}
      ...
      {isOpen && (
        <div
          ...
          className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-50 w-72 bg-white rounded-2xl border border-[#E2DDD3] shadow-lg p-3.5 animate-in fade-in zoom-in-95 duration-150"
        >
          {/* Navegação de Ano */}
          ...
          <div className="mt-2.5 pt-2 border-t border-stone-100">
            <button
              type="button"
              onClick={handleResetToCurrent}
              className="w-full py-1 text-xs font-semibold text-stone-600 hover:text-[#9E432A] hover:bg-[#FAF8F5] rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-1"
            >
              <Calendar className="w-3 h-3 text-[#9E432A]" />
              Voltar ao mês atual
            </button>
